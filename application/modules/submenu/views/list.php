<body class="hold-transition skin-blue sidebar-mini">
    ...
    <?php foreach ($total_result as $total_results): ?>
                                            <tr>
                                                ...
                                                <td><?= ++$count ?></td>
                                                <!--<td> <?php // pr($total_results);?></td>-->
                                                <td><?= ucfirst($total_results->menu_name); ?></td>
                                                <td><?= ucfirst($total_results->submenu_name); ?></td>
                                                <td><?= ucfirst($total_results->submenu_status); ?></td>    
                                                <td><?= $total_results->updated_at; ?></td>
                                                <td><?= $total_results->created_on; ?></td>
                                                <td>  <?php $id = ID_encode($total_results->submenu_id);
        echo anchor("submenu/view_listing/{$id}", 'View', ['class' => 'btn btn-info']) ?>
        <?php $id = ID_encode($total_results->submenu_id);
        echo anchor("submenu/delete/{$id}", 'Delete    ', ['class' => 'btn btn-danger']) ?>
