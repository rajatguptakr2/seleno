<body class="hold-transition skin-blue sidebar-mini">
    ...
    <?php foreach ($total_result as $total_results): ?>
                                            <tr>
                                                ...
                                                <td><?= ++$count ?></td>
                                                <td><?= ucfirst($total_results->set_name); ?></td>
                                                <td><?= ucfirst($total_results->set_type); ?></td>
                                                <td><?= ucfirst($total_results->set_status); ?></td>    
                                                <td><?= $total_results->updated_at; ?></td>
                                                <td><?= $total_results->created_on; ?></td>
                                                <td>  <?php $id = ID_encode($total_results->set_id);
        echo anchor("sets/view_listing/{$id}", 'View', ['class' => 'btn btn-info']) ?>
        <?php $id = ID_encode($total_results->set_id);
        echo anchor("sets/delete/{$id}", 'Delete    ', ['class' => 'btn btn-danger']) ?>
